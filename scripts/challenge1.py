...
def follow_yellow_line(contours_yellow, mask_yellow, image, w, vel, dec):
    ...
    err.append(abs(cx2-w/2))
    # Création du fichier txt contenant les valeurs de l'err pour un facteur de réduction de la vitesse angulaire de 40
    #np.savetxt('test1.challenge1.txt',err) # 1er test
    # Création du fichier txt contenant les valeurs de l'err pour un facteur de réduction de la vitesse angulaire de 60
    np.savetxt('test2.challenge1.txt',err) # 2eme test 
...
def image_callback(msg):
    ...
    vel = 0.12 #rospy.get_param("vel",0.12) 
